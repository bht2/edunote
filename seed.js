...
async function seed() {
  ...
  try {
    ...
    await db.execute('DROP TABLE IF EXISTS combo_requests');
    ...
    await db.execute(`CREATE TABLE combo_requests (
      id                 INT NOT NULL AUTO_INCREMENT,
    ...
    await db.execute(
      `INSERT IGNORE INTO admins (email, password, name, role) VALUES (?,?,?,'admin')`,
    ...
    // Education Levels
    const levels = [
      ["O'Level",       'olevel', "Ordinary Level — Senior 1 to 3",                '#10b981', '🎓', 1],
      ["A'Level",       'alevel', "Advanced Level — Senior 4 to 6",                '#6366f1', '📘', 2],
    ...
    const [[olevelRow]] = await db.execute("SELECT id FROM education_levels WHERE slug='olevel'");
    const [[alevelRow]] = await db.execute("SELECT id FROM education_levels WHERE slug='alevel'");
    const [[tvetRow]]   = await db.execute("SELECT id FROM education_levels WHERE slug='tvet'");

    // O'Level — single combined combo S1-S3
    await db.execute(
      'INSERT IGNORE INTO combinations (education_level_id,name,slug,full_name,color,order_index) VALUES (?,?,?,?,?,?)',
      [olevelRow.id, "O'Level", 'olevel-general', "General — S1 to S3", '#10b981', 1]
    );
    const [[olCombo]] = await db.execute("SELECT id FROM combinations WHERE slug='olevel-general'");
    for (const [i, cls] of ['S1','S2','S3'].entries()) {
      await db.execute(
        'INSERT IGNORE INTO classes (combination_id,name,slug,order_index) VALUES (?,?,?,?)',
        [olCombo.id, cls, `olevel-${cls.toLowerCase()}`, i+1]
      );
    }
    ...
    for (const [i, [name,slug,fullName,color]] of aLevelCombos.entries()) {
      await db.execute(
        'INSERT IGNORE INTO combinations (education_level_id,name,slug,full_name,color,order_index) VALUES (?,?,?,?,?,?)',
        [alevelRow.id, name, slug, fullName, color, i+1]
      );
    ...
    for (const [i, [name,slug,fullName,color]] of tvetTrades.entries()) {
      await db.execute(
        'INSERT IGNORE INTO combinations (education_level_id,name,slug,full_name,color,order_index) VALUES (?,?,?,?,?,?)',
        [tvetRow.id, name, slug, fullName, color, i+1]
      );
      const [[combo]] = await db.execute('SELECT id FROM combinations WHERE slug=?', [slug]);
    ...
    console.log(`\n🎉 Done!`);
    console.log(`   Login: ${process.env.ADMIN_EMAIL || 'admin@edunote.com'} / ${process.env.ADMIN_PASSWORD || 'Admin@123'}`);
    await db.end();
    process.exit(0);
  } catch (err) {
